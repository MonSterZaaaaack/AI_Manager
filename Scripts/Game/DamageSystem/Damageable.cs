...
namespace Gamekit3D
{
    public partial class Damageable : MonoBehaviour
    {
        ...
        public List<MonoBehaviour> onDeathMessageReceivers;
        ...
        public void ApplyDamage(DamageMessage data)
        {
            ...
            if (data.damager.GetComponentInParent<ChomperBehavior>() != null)
            {
                var newDamager = data.damager.GetComponentInParent<ChomperBehavior>();
                if (!onDeathMessageReceivers.Contains(newDamager))
                {
                    onDeathMessageReceivers.Add(newDamager);
                }
            }
            else if(data.damager.GetComponentInParent<SpitterBehaviour>() != null)
            {
                var newDamager = data.damager.GetComponentInParent<SpitterBehaviour>();
                if (!onDeathMessageReceivers.Contains(newDamager))
                {
                    onDeathMessageReceivers.Add(newDamager);
                }
            }
            else if(data.damager.GetComponentInParent<GrenadierBehaviour>() != null)
            {
                var newDamager = data.damager.GetComponentInParent<GrenadierBehaviour>();
                if (!onDeathMessageReceivers.Contains(newDamager))
                {
                    onDeathMessageReceivers.Add(newDamager);
                }
        ...
        {
            for(var i = 0; i < onDeathMessageReceivers.Count; ++i)
            {
                var receiver = onDeathMessageReceivers[i] as IMessageReceiver;
                receiver.OnReceiveMessage(MessageType.TARGETDEAD, gameObject, 0);
